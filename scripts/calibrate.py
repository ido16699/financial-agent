...
def main(ticker: str, as_of_str: str | None, skip_external: bool) -> None:
    ...
    print(f"  μ (ann.) = {params.mu*100:.3f}%")
    print(f"  σ (ann.) = {params.sigma*100:.3f}%")
    print(f"  P(up) baseline (closed-form) = {p_up_base*100:.1f}%\n")

    # --- External signals ---
    if skip_external:
        sentiment, trends_z = 0.0, 0.0
        print("External signals: SKIPPED (zeroed)\n")
    else:
        print("Fetching news sentiment...")
        sentiment = fetch_news_sentiment(ticker)
        print("Fetching Google Trends z-score...")
        trends_z = fetch_trends_zscore(ticker)
        print(f"  Sentiment (VADER) = {sentiment:.4f}")
        print(f"  Trends z-score    = {trends_z:.4f}\n")

    # --- Perturbation ---
    fc = compute_forecast(params, sentiment=sentiment, trends_zscore=trends_z)

    print("Perturbation Series:")
    print(f"  μ₀  = {fc.mu_0*100:.4f}%  (raw GBM drift)")
    print(f"  Δ₁  = {fc.delta_1*100:.4f}%  (first-order)")
    print(f"  Δ₂  = {fc.delta_2*100:.4f}%  (second-order)")
    print(f"  μ*  = {fc.mu_perturbed*100:.4f}%  (perturbed drift)\n")
